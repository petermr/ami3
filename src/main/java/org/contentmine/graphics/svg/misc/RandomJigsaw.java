...
public class RandomJigsaw {
	...
	public static void main(String[] args) {
		RandomJigsaw quote = new RandomJigsaw();
		SVGG g;
	}
	...
	private void drawRect() {
		...
		for (int j = 0; j <= NY; j++) {
			for (int i = 0; i <= NX; i++) {
				xy[i][j] = new Real2(createX(i), createY());
			}
		}
		...
		for (int j = 0; j < NY; j++) {
			for (int i = 0; i < NX; i++) {
				...
				if (i < NX && j > 0) {
					int currentX = (i) * DX;
					addLine(
							new Real2(OFFSETX + currentX, OFFSETY + j * DY),
							new Real2(OFFSETX + currentX, OFFSETY + (j + 1) * DY),
							new Angle(0.0),
