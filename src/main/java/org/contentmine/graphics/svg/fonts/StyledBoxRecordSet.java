...
public class StyledBoxRecordSet implements Iterable<StyleRecord> {
	...
	private List<Entry<Integer>> xBoxEndListSortedByCount;
	private List<Entry<Integer>> xBoxEndBundleCounts;
	private List<Entry<Integer>> xBoxStartSortedByCount;
	private List<Entry<Integer>> xBoxStartBundleCounts;

	public StyledBoxRecordSet() {
	 ...
	 * @return
	 */
	public StyledBoxRecordSet getStyleRecordSet(String attName, String attValue) {
		StyledBoxRecordSet styleRecordSet = new StyledBoxRecordSet();
...
// 	}
	...
	public SVGElement createStyledTextBBoxes(List<SVGText> svgTexts) {
		...
		Multiset<Integer> xBoxStarts = HashMultiset.create();
		Multiset<Integer> xBoxEnds = HashMultiset.create();
		for (SVGText svgText : svgTexts) {
			SVGRect rect = createAndStyleBox(svgText);
			g.appendChild(createTextBox(svgText, rect));
			
			Real2Range bbox = rect.getBoundingBox();
			xBoxStarts.add((int)(double)bbox.getXMin());
			xBoxEnds.add((int)(double)bbox.getXMax());
		}
		int tolerance = 3;
		createXBoxStarts(g, xBoxStarts, tolerance);
		createXBoxEnds(g, xBoxEnds, tolerance);
		return g;
	}


	private void createXBoxEnds(SVGElement g, Multiset<Integer> xBoxEnds, int tolerance) {
		xBoxEndListSortedByCount = MultisetUtil.createListSortedByCount(xBoxEnds);
		xBoxEndBundleCounts = MultisetUtil.bundleCounts(xBoxEndListSortedByCount, tolerance);
		drawWeightedVerticalLines(g, xBoxEndListSortedByCount, "green");
		drawWeightedVerticalLines(g, xBoxEndBundleCounts, "cyan");
	}


	private void createXBoxStarts(SVGElement g, Multiset<Integer> xBoxStarts, int tolerance) {
		xBoxStartSortedByCount = MultisetUtil.createListSortedByCount(xBoxStarts);
		xBoxStartBundleCounts = MultisetUtil.bundleCounts(xBoxStartSortedByCount, tolerance);
		drawWeightedVerticalLines(g, xBoxStartSortedByCount, "blue");
		drawWeightedVerticalLines(g, xBoxStartBundleCounts, "purple");
	}


	private SVGRect createAndStyleBox(SVGText svgText) {
		...
		String fill = (StyleBundle.ITALIC.equals(fontStyle)) ? ITA : "#ffcccc";
	...
	private SVGG createTextBox(SVGText svgText, SVGRect rect) {
		SVGG gg = new SVGG();
		gg.appendChild(rect);
