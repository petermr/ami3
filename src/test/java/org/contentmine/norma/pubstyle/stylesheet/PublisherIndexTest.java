...
import org.contentmine.norma.pubstyle.stylesheet.PublisherIndex;
import org.contentmine.norma.pubstyle.stylesheet.PublisherSelector;
...
public class PublisherIndexTest {
	...
	private static final Logger LOG = LogManager.getLogger(PublisherIndexTest.class);
@Test
	...
	public void testIdentifyDocument1() {
		File fulltext = new File(NormaFixtures.TEST_PUBSTYLE_DIR + "/acs/out/clean/jo402790x/fulltext.xhtml");
		PublisherIndex publisherIndex = new PublisherIndex(NormaFixtures.MAIN_PUBSTYLE_DIR);
	...
	public void testIdentifyDocuments() {
		...
		for (File fulltextXhtmlFile : fulltextXhtmlFiles) {
			...
			LOG.trace(fulltextXhtmlFile.getParentFile().toString().replace(NormaFixtures.TEST_PUBSTYLE_DIR+"/", "")+"; "+publisherSelector);
